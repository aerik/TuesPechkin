...
{
    public class ToolsetConverter<AToolset> : MarshalByRefObject, IConverter, IDisposable where AToolset : MarshalByRefObject, IToolset
    {
        protected AToolset Toolset { get; private set; }

        protected IDocument ProcessingDocument { get; private set; }

        public ToolsetConverter(Func<AToolset> toolsetFactory)
        {
            ...
            Tracer.Trace(string.Format("T:{0} Created ToolsetConverter", Thread.CurrentThread.Name));
        }
        ...
        public virtual void Close()
        {
            lock (_ConverterLocker)
            {
                if (_ConvertPtr != null && _ConvertPtr != IntPtr.Zero)
                {
                    //"destroy"ing the converter doesn't stop the conversion process
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ...
                if(Toolset != null)
                {
                    if (Toolset is IDisposable)
                    {
                        ((IDisposable)Toolset).Dispose();
                    }
                    else
                    {
                        Toolset.Unload();
